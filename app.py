...
def model(text, web_search):
    if web_search is True:
        ...
        formatted_prompt = system_instructions1 + text + "[WEB]" + str(web2) + "[OnurGPT 76.54]"
        ...
        formatted_prompt = system_instructions1 + text + "[OnurGPT 76.54]"
        stream = client1.text_generation(formatted_prompt, max_new_tokens=512, stream=True, details=True, return_full_text=False)
        return "".join([response.token.text for response in stream if response.token.text != "</s>"])
...
with gr.Blocks() as instant:
    gr.HTML("<iframe src='https://auguriant-sdxl-flash.hf.space' width='100%' height='2000px' style='border-radius: 8px;'></iframe>")

with gr.Blocks() as dalle:
    gr.HTML("<iframe src='https://auguriant-image-gen-pro.hf.space' width='100%' height='2000px' style='border-radius: 8px;'></iframe>")
...
with gr.Blocks() as instant2:
    gr.HTML("<iframe src='https://auguriant-instant-video.hf.space' width='100%' height='3000px' style='border-radius: 8px;'></iframe>")
...
with gr.Blocks(theme=theme, title="OnurGPT 76.54 DEMO") as demo:
    gr.Markdown("# OnurGPT 76.54")
